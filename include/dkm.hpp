...
//** Customized one for optimal grasp framework **//

template <typename T, size_t N>
T distance_squared(const std::array<T, N>& _point_a, const std::array<T, N>& _point_b)
{
    Eigen::Vector3f pos_a(_point_a[0], _point_a[1], _point_a[2]);
    Eigen::Vector3f pos_b(_point_b[0], _point_b[1], _point_b[2]);

    Eigen::Vector3f norm_a(_point_a[3], _point_a[4], _point_a[5]);
    Eigen::Vector3f norm_b(_point_b[3], _point_b[4], _point_b[5]);

    Eigen::Vector3f diffPos  = pos_a - pos_b;
    Eigen::Vector3f diffNorm = norm_a - norm_b;

    Eigen::Vector3f sumPosNorm = diffPos + (ETA_NORMAL_WEIGHT * diffNorm);

    T d_squared = T();
    d_squared = Utilities::vecMagnitude3D(sumPosNorm[0], sumPosNorm[1], sumPosNorm[2]);
